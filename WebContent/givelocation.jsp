...
String uemail=session.getAttribute("uemail").toString();
 ...
 while(rs1.next())
 {
	 ...
	 String vdemail=rs1.getString(5);
       ...
       con=Dbcon.create();
       PreparedStatement ps=con.prepareStatement("SELECT * FROM `humansecurity`.`driverapplication` where email='"+vdemail+"' and status='Accept'  ");
...
<div class="card">
  ...
  <%--   <p class="card-text">License: <%=rs.getString(8)%></p> --%>
    ...
    <a href="filltoaddress.jsp?id=<%=rs.getString(1)%>&&uemail=<%=uemail%>&&demail=<%=rs.getString(4)%>"><button type="button" class="btn btn-primary">Request</button></a>  
